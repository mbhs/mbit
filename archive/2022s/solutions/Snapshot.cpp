...
int main(){
  ...
  vl a(n);
  forn(i,n) cin >> a[i];
  int k; cin >> k;//time interval for first snapshot
  ...
  forn(i,2*n){
  	if(s[i].s==-1) occs[s[i].f].push(i);
  	else occr[s[i].f].push(i);
  }
  ...
  forn(i,2*n){
  	if(taken[i]) continue;
  	if(s[i].s==-1){
  		if(occr[s[i].f+k].size()==0) mb=false;
  		else{
        ...
        occr[s[i].f+k].pop();
  		...
  		if(occs[s[i].f+k].size()==0) mb=false;
  		else{
  			b[s[i].s]=1;
