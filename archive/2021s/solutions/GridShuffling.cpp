...
void rotate(State& g) {
    ...
    for (int x = 0; x < n / 2; ++x) {
        for (int y = x; y < n - x - 1; ++y) {
            int tmp = g[x][y];
            g[x][y] = g[y][n - 1 - x];
            g[y][n - 1 - x] = g[n - 1 - x][n - 1 - y];
            g[n - 1 - x][n - 1 - y] = g[n - 1 - y][x];
            g[n - 1 - y][x] = tmp;
        }
    }
}
...
template <class T> int get_pos(V<T>& v, T x) {
...
int main() {
    ...
    f0r(i, sz) {
        int to = done[i];
        use[v[to].f][v[to].s] = now[v[i].f][v[i].s];
    }
